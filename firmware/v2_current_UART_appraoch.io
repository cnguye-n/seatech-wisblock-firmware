...
struct GnssSnapshot {
  ...
  GnssSnapshot()
    : fixType(-1), siv(-1), lat(0), lon(0), altMm(0), surfaceFix(false),  //NEW: altMm
...
void tryInitSD() {
  ...
  if (!SD.exists(CSV_PATH)) {
    ...
    if (file) {
      file.println("utc_iso,latitude,longitude,altitude_m,surfaceFix,fixType,siv,uptime_min,batt_v,batt_pct"); //NEW
      file.close();
...
GnssSnapshot getGnssSnapshot() {
  ...
  while (millis() - start < GNSS_ATTEMPT_MS) {
    ...
    if (fix >= 2 && siv >= MIN_SIV_FOR_VALID) {
      ...
      snapshot.altMm = myGNSS.getAltitude(); //NEW: capture altitude in mm
...
void loop() {
  ...
  if (millis() - lastLog >= LOG_INTERVAL_MS) {
    ...
    if (gSnapshot.surfaceFix) {
      Serial.print(gSnapshot.altMm / 1000.0f, 2); //NEW: mm -> meters
    }
#endif
    Serial.println();
    ...
    if (sdOK) {
      ...
      if (file) {
        ...
        // 2) latitude, longitude, altitude_m  //NEW
        if (gSnapshot.surfaceFix) {
          float latDeg = gSnapshot.lat / 10000000.0f;
          float lonDeg = gSnapshot.lon / 10000000.0f;
          float altM = gSnapshot.altMm / 1000.0f; //NEW: mm -> meters
          file.print(latDeg, 7);
          file.print(",");
          file.print(lonDeg, 7);
          file.print(",");
          file.print(altM, 2); //NEW
          file.print(",");
        } else {
          file.print(",");
          file.print(",");
          file.print(","); //NEW: blank altitude too
        }
